...
interface ScreenState {
    ...
    updateScreen: (scrn: Screen) => Promise<void>;
...
const useScreenStore = create<ScreenState>((set, get) => ({
    ...
    getScreensByChapterId: async (chapterId: string) => {
        ...
        const screensList = await Promise.all(scrns.map(async (scrn) => {
            if (scrn.image) {
                const fileRef = ref(storage, scrn.image);
                const url = await getDownloadURL(fileRef);
                return { ...scrn, imageLocal: url };
    ...
    updateScreen: async(scrn: Screen) => {
        const updated = get().screens.map(screen =>
            screen.id === scrn.id ? { ...screen, ...scrn } : screen
        );
        console.log( updated );
        set((state) => ({
            ...state,
