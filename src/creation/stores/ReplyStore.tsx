...
interface ReplyState {
  ...
  updateReply: (reply: Reply, save?: boolean) => Promise<void>;
}
...
const useReplyStore = create<ReplyState>((set, get) => ({
  ...
  updateReply: async (reply: Reply, save?: boolean) => {
    const { repliesMap } = get();
    const replyList = [...repliesMap[reply.screenId]];
    const updated = replyList.map((repl) => (repl.id === reply.id ? { ...repl, ...reply } : repl));
    set((state) => ({
      ...state,
